...
class Solution {
  ...
  void help(Node*root){
      ...
      if(first==NULL and prev->data >= root->data){ // first violation
          first=prev;// since it's left side, violated node will be prev
      }
      if(first!=NULL and prev->data >= root->data){ // second violation
          second=root; // since it's right side, violated node will be root
      }
      }
      prev=root;
      help(root->right);
